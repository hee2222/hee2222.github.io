...
$to = "lmj@readyfor.co.kr, kmj@readyfor.co.kr";
$from = "readyfor@readyfor.dothome.co.kr";


$boundary = "----" . uniqid("part");

$header = "From:readyfor<$from>\n";
...
$form5_result = join(", ", $_POST["contact-form5"]);
...
if ($object == 'member') {
	...
	if (in_array("metaverse", $_POST["contact-form2"])) {
		$msg .= '메타버스 종류 : ' . $form5_result  . '<br>';
	}
	...
	$msg .=  '예산 : ' . $_POST["contact-form7"] . '<br><br>';
	$msg .= '기본 정보 <br>';
...
if (mail($to, $title, $mailbody, $header)) {
	header("Location: ./index.html#ready-contact-last-section");
	return true;
} else {
	echo '전송 실패!';
	return false;
